...
- Components (`src/components/`):
  ...
  - `FocusMode.vue` — full-screen execution view (view mode `focus`): a 4-panel carousel (UP NEXT / IN PROGRESS & QUEUED / NOW / DONE) pulled from SELECTED and WIP columns only. A task is **on deck** if staged into WIP or due by the Saturday closing this Sunday–Saturday week. IN PROGRESS / QUEUED combines underway work and unstarted work due later this week; NOW holds the remaining unstarted deck plus every task due today, regardless of status, and all overdue work. Both execution panels group cards by Today (including overdue), General, then each upcoming due day. UP NEXT keeps the rest of SELECTED and DONE keeps completed SELECTED/WIP work. A sliding 2-wide spotlight keeps adjacent panels fully visible; off-window panels peek from the edges. Cards carry their board section as a badge and cluster by section within each due day. Clicking a task name edits the name, while clicking its clock-marked due-date badge separately edits or clears the due date; both preserve notes/completion metadata. Labeled `Start work` and `Back to queue` actions replace icon-only status toggles; start/reopen pull non-WIP tasks into active WIP, and header quick-add appends unstarted NOW work. Cross-panel moves are staged with a hold, flight, and landing highlight. Navigation supports panel clicks, dots, arrow keys, and horizontal swipe. Theme follows the dark/light/auto toolbar preference (`localStorage.themePreference`).
