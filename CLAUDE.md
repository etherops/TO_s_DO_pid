...
- Components (`src/components/`):
  ...
  - `FocusMode.vue` — full-screen execution view (view mode `focus`) pulled from SELECTED and WIP. The top is a three-panel carousel with exactly one spotlight: UP NEXT / WAITING on the left, NOW enlarged in the center by default, and IN PROGRESS / QUEUED in the former DONE position on the right. Both side panels remain fully visible but distinctly shorter and narrower than the spotlight. A task is **on deck** if staged into WIP or due by the Saturday closing this Sunday–Saturday week. IN PROGRESS / QUEUED holds non-urgent active work and non-urgent dated queued work; NOW holds undated queued work, overdue/due-today work regardless of status, and work completed or cancelled today. Future dated queued work appears in both IN PROGRESS / QUEUED and its weekday below. DONE is replaced by an always-visible full-width Sunday-Saturday “Week at a glance” strip. Future days show scheduled work; past days retain only completed/cancelled work, falling back to the completion stamp when no due date exists. Each bounded day pane scrolls independently. All weekly cards use a compact treatment without redundant due-date badges: the section is a hoverable one-letter marker, notes collapse to an icon, and an icon-only clock retains due-date editing. Today keeps the same width and shows a lightly muted duplicate of every card in NOW; its clock remains interactive while the rest of the mini card is display-only. Inside each group, cards use the board's completed → cancelled → in-progress → unstarted order, then section clustering. Task-name and clock-badge editing preserve notes/completion metadata. The four-state control updates immediately but re-bucketing is debounced for 1.5 seconds so repeated clicks do not move or re-sort the card under the pointer. Actual moves use the hold/flight/landing treatment; unchanged slots do not whisk. Navigation supports panel clicks, compact dots, arrow keys, and horizontal swipe. Theme follows the toolbar preference.
