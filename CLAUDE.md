...
- Components (`src/components/`):
  ...
  - `FocusMode.vue` — full-screen execution view (view mode `focus`): a 4-panel carousel (UP NEXT / IN PROGRESS & QUEUED / NOW / DONE) pulled from SELECTED and WIP columns only. A task is **on deck** if staged into WIP or due by the Saturday closing this Sunday–Saturday week. IN PROGRESS / QUEUED holds non-urgent work already underway; NOW holds every unstarted on-deck task, all overdue and due-today work regardless of status, and anything completed or cancelled today. When later-this-week work exists, a compact, content-height, roughly two-thirds-width “Up next this week” shelf sits at the bottom of the stage; each future due day gets its own horizontal subcolumn, while the main carousel expands to consume all remaining height above it. Compact carousel dots sit immediately below the filled stage. Inside each existing group, cards use the main board's completed → cancelled → in-progress → unstarted status order, with section clustering as the final tie-breaker. UP NEXT keeps the rest of SELECTED, including undated SELECTED tasks marked in progress, and DONE keeps older completed/cancelled SELECTED/WIP work. A sliding 2-wide spotlight keeps adjacent panels fully visible; off-window panels peek from the edges. Cards carry their board section as a badge. Clicking a task name edits the name, while clicking its clock-marked due-date badge separately edits or clears the due date; both preserve notes/completion metadata. The card checkbox cycles through the same four statuses as the board: queued, in progress, completed, and cancelled. Like the board control, its visual state changes immediately but re-bucketing is debounced for 1.5 seconds so repeated clicks do not move or re-sort the card under the pointer. Moves to another panel, list position, or due-day section then use the hold/flight/landing treatment; edits that leave the card in the same rendered slot do not whisk. Navigation supports panel clicks, dots, arrow keys, and horizontal swipe. Theme follows the dark/light/auto toolbar preference (`localStorage.themePreference`).
